...
import (
	...
	"k8s.io/kubernetes/pkg/kubectl/dispatcher/pkg/client"
	"k8s.io/kubernetes/pkg/kubectl/dispatcher/pkg/filepath"
	"k8s.io/kubernetes/pkg/kubectl/dispatcher/pkg/util"
...
func (d *Dispatcher) InitKubeConfigFlags() (*genericclioptions.ConfigFlags, error) {
	...
	kubeConfigFlagSet.VisitAll(func(flag *pflag.Flag) {
		glog.V(5).Infof("KubeConfig Flag: --%s=%q", flag.Name, flag.Value)
	})
...
func (d *Dispatcher) Dispatch() error {
	...
	glog.V(5).Infof("Server Version: %s", serverVersion.GitVersion)
	glog.V(5).Infof("Client Version: %s", d.GetClientVersion().GitVersion)
	...
	glog.V(5).Infof("kubectl dispatching: %s\n", kubectlFilepath)
...
func Execute(clientVersion version.Info) {
	glog.V(5).Info("Starting dispatcher")
	...
	if err := dispatcher.Dispatch(); err != nil {
		glog.V(2).Infof("Dispatch error: %v", err)
	}
}
